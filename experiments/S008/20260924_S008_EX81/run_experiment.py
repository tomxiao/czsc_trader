...
def main() -> None:
    ...
    for name in ("artifacts", "03_execution.md", "04_conclusion.md", "experiment_manifest.json"):
        if (experiment_root / name).exists():
            raise FileExistsError(f"EX81 already has immutable output: {name}")
    if sha256((experiment_root.parent / "20260924_S008_EX80" / "experiment_manifest.json").read_bytes()).hexdigest() != FAILED_MANIFEST_SHA256:
    ...
    (experiment_root / "03_execution.md").write_text(
        ...
        "EX80 源码与失败清单经哈希锁定，唯一修复为 EX79 receipt 字面值。"
    ...
    build_experiment_manifest(experiment_root, {
        ...
        "technical_predecessor_experiment_id": "20260924_S008_EX80",
        "technical_predecessor_manifest_sha256": FAILED_MANIFEST_SHA256,
