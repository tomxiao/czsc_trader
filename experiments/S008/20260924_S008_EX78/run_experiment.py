...
def main() -> None:
    ...
    for name in ("artifacts", "03_execution.md", "04_conclusion.md", "experiment_manifest.json"):
        if (experiment_root / name).exists():
            raise FileExistsError(f"EX78 already has immutable output: {name}")
    ...
    (experiment_root / "03_execution.md").write_text(
        "# S008 EX78 执行\n\n"
        f"REX receipt=`{result.receipt.sha256}`；EX68 前序 receipt=`{predecessor.receipt_sha256}`。"
        "只按身份读取已有 EX42、EX48、EX16 开发期归档；无新市场数据请求、密封区读取、参数搜索或候选。\n",
        encoding="utf-8",
    )
    (experiment_root / "04_conclusion.md").write_text(
        ...
        "完整地图与 T-1/T-21 可观测值见 `artifacts/`。Oracle 是事后上界，转换样本不足以学习或验证规则。"
        "本实验不授予原型或候选资格。\n",
        encoding="utf-8",
    )
