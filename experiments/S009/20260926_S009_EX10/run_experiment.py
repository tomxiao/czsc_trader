...
def main() -> None:
    ...
    (experiment_root / "03_execution.md").write_text(
        "# S009 EX10 执行\n\n"
        f"REX receipt=`{result.receipt.sha256}`；EX09前序receipt=`{predecessor.receipt_sha256}`。"
        "按冻结的四项年度向前比较完成政策不确定性增量、Bootstrap、多重校正和冗余审查；"
        "仅使用截至2024-12-31的已见开发池，未读取封存验证。\n",
        encoding="utf-8",
    )
    (experiment_root / "04_conclusion.md").write_text(
        "# S009 EX10 结论\n\n"
        f"机器裁决：`{facts['decision']}`；新增组件合格={facts['new_component_eligible']}；"
        f"组件总数={facts['component_count']}；主比较DeltaMSE={facts['primary_delta_mse']:.12f}；"
        f"正向年份={facts['primary_positive_years']}/6；q={facts['primary_q_value']:.6f}；"
        f"最大比较变量相关={facts['maximum_abs_comparator_correlation']:.6f}。"
        "本轮属于已见开发池内的事后机制正式化，不授予独立验证、策略原型、候选或硬目标达成资格。\n",
        encoding="utf-8",
    )
    build_experiment_manifest(experiment_root, {
        "experiment_id": loaded.definition.experiment_id,
        "status": "COMPLETE",
        "experiment_type": "s009_policy_uncertainty_incremental_information_audit",
        "strategy_id": "S009",
