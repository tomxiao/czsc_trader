...
def main() -> None:
    ...
    (experiment_root / "03_execution.md").write_text(
        "# S009 EX05 执行\n\n"
        f"REX receipt=`{result.receipt.sha256}`；EX04前序receipt=`{predecessor.receipt_sha256}`。"
        "两只宽基ETF份额仅经DFLS读取；未读取518880收益或封存验证。\n",
        encoding="utf-8",
    )
    (experiment_root / "04_conclusion.md").write_text(
        "# S009 EX05 结论\n\n"
        f"机器裁决：`{facts['decision']}`；共同日历={facts['common_calendar']}；"
        f"问题={json.dumps(facts['issues'], ensure_ascii=False)}。通过只授予后继信息复核资格。\n",
        encoding="utf-8",
    )
    build_experiment_manifest(experiment_root, {
        "experiment_id": loaded.definition.experiment_id,
        "status": "COMPLETE",
        "experiment_type": "s009_broad_equity_liquidity_data_gate",
        "strategy_id": "S009",
