...
def main() -> None:
    ...
    context = create_experiment_context(
        ...
        resources=ExperimentResources(max_workers=1, random_seed=loaded.definition.random_seed), predecessors=(predecessor,),
    )
    ...
    (experiment_root / "03_execution.md").write_text(
        "# S009 EX07 执行\n\n"
        f"REX receipt=`{result.receipt.sha256}`；EX06前序receipt=`{predecessor.receipt_sha256}`。"
        "仅经DFLS读取159915与参考日历份额，未读取目标收益。\n", encoding="utf-8",
    )
    (experiment_root / "04_conclusion.md").write_text(
        "# S009 EX07 结论\n\n"
        f"机器裁决：`{facts['decision']}`；缺口数={facts['missing_count']}；"
        f"问题={json.dumps(facts['issues'], ensure_ascii=False)}。通过只授予第三代理复核资格。\n", encoding="utf-8",
    )
    build_experiment_manifest(experiment_root, {
        "experiment_id": loaded.definition.experiment_id, "status": "COMPLETE",
        "experiment_type": "s009_third_equity_proxy_data_gate", "strategy_id": "S009",
